...
;; + lab group participant LiU-ID: filjo653
...
;***********************************************
;*
;* GPIO D interrupt MUX
;*
intgpiod:
    ; Återställ avbrottsflaggan
    mov r0,#(GPIOD_GPIOICR & 0xffff)
    movt r0,#(GPIOD_GPIOICR >> 16)
    mov r1,#(1<<7)
    str r1,[r0]

    ; ladda positionen på nästa siffra
    mov r12,#(0x20001004 & 0xffff)
    movt r12,#(0x20001004 >> 16)
    ldrb r1, [r12]

    ; skicka positionen till displayen
    mov  r0,#(GPIOF_GPIODATA & 0xffff)
    movt r0,#(GPIOF_GPIODATA >> 16)
    str r1, [r0]

    ; tar fram värdet på siffran som ska skrivas ut
    mov r0,#(0x20001000 & 0xffff)
    movt r0,#(0x20001000 >> 16)
    add r0, r0, r1
    ldrb r0, [r0]
    ...
    ; antar att den finns i R0 h¨?ar
    ADR r2,SJUSEGTAB ; Tabellstart till R2
    ADD r2,r2,r3 ; Peka ut r¨?att byte
    LDRB r3,[r2] ; H¨?amta bitm¨?onstret

    ; skicka det utmaskade värdet på siffran till displayen
...
intgpioe:
    ...
    ; titta om sekundvisaren har värdet 9, då ska den slå tillbaka till noll osv annars +1
    ldrb r1, [r0]
    cmp r1, #9
    beq addten
...
addten: ;+1 till 10-sekundvisaren
    ; sätt sekundvisaren till noll
    mov r1, #0
    strb r1, [r0]

    ldrb r1, [r0, #1]

    ; titta om minutvisaren är 5, för då ska den slå över. Annars +1
    cmp r1, #5
    beq addmin
...
addmin: ;+1 till minutvisaren, precis samma struktur som dem andra klockfunktionerna
    mov r1, #0
    strb r1, [r0, #1]
    ldrb r1, [r0, #2]
...
addtenmin: ; +1 till 10-minutvisaren, precis samma struktur som de andra klockfunktionerna
    mov r1, #0
    strb r1, [r0, #2]
    ldrb r1, [r0, #3]
...
resetmin: ; klockan har nått 60 minuter, återställ
    mov r1, #0
    strb r1, [r0, #3]
